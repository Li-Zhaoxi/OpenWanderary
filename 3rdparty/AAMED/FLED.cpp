...
void FLED::run_FLED(Mat Img_G, bool needGaussBlur)
{
	// Pre-Processing
#ifdef DETAIL_BREAKDOWN
	double tmp_st;
	...
	if (needGaussBlur)
	{
		GaussianBlur(Img_G, imgT, cv::Size(2 * L + 1, 2 * L + 1), var);
	}
	else
		imgT = Img_G;
	...
	//Canny����������������Ӧ
	int low, high;
	calCannyThreshold(imgT, low, high);
	Canny(imgT, imgCanny, low, high);
