...
public:
	...
	void run_FLED(Mat Img_G, bool needGaussBlur = true);
...
public: // The public functions that used for Python.
	int run_FLED(unsigned char* _img_G, int irows, int icols, int needGaussBlur)
	{
		Mat Img_G(irows, icols, CV_8UC1, _img_G);
		run_FLED(Img_G, needGaussBlur? true: false);
	...
	void drawAAMED(unsigned char* _img_C, int irows, int icols)
	{
		Mat img_C = Mat(irows, icols, CV_8UC3, _img_C);
		drawFLED(img_C, "");
		if (img_C.data != _img_C)
		{
			memcpy(_img_C, img_C.data, sizeof(unsigned char) * irows * icols);
		}
	}
...
protected:
	...
	void BoldEdge(uchar *_edge, const vector< vector<cv::Point> > &edge_Contours);
	void FSA_Segment(vector<cv::Point> &dpContour);


	void Arcs_Grouping(vector< vector<cv::Point> > &fsaarcs);
	void CreateArcSearchRegion(vector< vector<cv::Point> > &fsaarcs);

	vector< vector<cv::Point> > edgeContours, dpContours, FSA_ArcContours;
	...
	void SegmentAnArc(vector<cv::Point> &AdpContour);
	vector< vector<cv::Point> > StrongArcs;
	vector< vector<cv::Point> > WeakArcs;
	void SortArcs(vector<vector<cv::Point>> &detArcs);
...
private:
	...
	void clockwiseContour(vector<cv::Point> &antiContour);

	vector<cv::Point> oneContour;
	vector<cv::Point> oneContourOpp;

	Node_FC *data;
	// KD-Trees. KD is used for high speed
	void getArcs_KDTrees(vector< vector<cv::Point> > & arcs);
	...
	void GetArcMat(vector< cv::Vec<double, MAT_NUMBER> > &arcfitmat, vector< vector<cv::Point> > &fsa_arc);
