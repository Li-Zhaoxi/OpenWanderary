...
void Simplified_EDLines::setParameters(int min_line_length, double lineFitErrThreshold)
{
...
void Simplified_EDLines::runSimplified_EDLines(cv::Mat &ImgG, bool need_gaussblur)
{
	...
	if (need_gaussblur)
	{
		...
		GaussianBlur(ImgG, imgT, cv::Size(2 * L + 1, 2 * L + 1), var);
	}
	else
		imgT = ImgG;

	int canny_high, canny_low;
	calCannyThreshold(imgT, canny_low, canny_high);
	cv::Canny(imgT, imgCanny, canny_low, canny_high);
...
void Simplified_EDLines::drawLineSegments4i(cv::Mat &ImgG, bool isShow)
{
	...
	else
	{
		...
		cvtColor(ImgT, ImgT, cv::COLOR_GRAY2BGR);
		for (int i = 0; i < LineSegments.size(); i++)
		{
			cv::Point st, ed;
			st.y = LineSegments[i][0], st.x = LineSegments[i][1];
...
void Simplified_EDLines::UpdateLineSegments(signed int *_linesegs)
{
	int linenum = LineSegments.size();
	for(int i = 0; i < linenum; i++)
	{
		auto &lineseg = LineSegments[i];
		_linesegs[i * 4 + 0] = lineseg.val[0];
		_linesegs[i * 4 + 1] = lineseg.val[1];
		_linesegs[i * 4 + 2] = lineseg.val[2];
		_linesegs[i * 4 + 3] = lineseg.val[3];
	}
...
void drawLineSegments(cv::Mat &imgC, std::vector<cv::Vec4i> &LineSegments, cv::Scalar color, int thickness)
{
	if(imgC.channels() == 1)
		cvtColor(imgC, imgC, cv::COLOR_GRAY2BGR);
	
	for(auto &eachline: LineSegments)
	{
		cv::Point st, ed;
		st.x = eachline[0], st.y = eachline[1];
		ed.x = eachline[2], ed.y = eachline[3];

		cv::line(imgC, st, ed, color, thickness);

	}

}

void drawLineSegments(unsigned char *_imgC, int irows, int icols, signed int *_LineSegents, int linenum, unsigned int *_color, int thickness)
{
	if (linenum == 0)
		return;
	
	cv::Mat imgC(irows, icols, CV_8UC3, _imgC);
	std::vector<cv::Vec4i> LineSegments(linenum);
	for(int i = 0; i < linenum; i++)
	{
		LineSegments[i][0] = _LineSegents[i * 4 + 0];
		LineSegments[i][1] = _LineSegents[i * 4 + 1];
		LineSegments[i][2] = _LineSegents[i * 4 + 2];
		LineSegments[i][3] = _LineSegents[i * 4 + 3];
	}

	cv::Scalar color(_color[0], _color[1], _color[2]);

	drawLineSegments(imgC, LineSegments, color, thickness);
