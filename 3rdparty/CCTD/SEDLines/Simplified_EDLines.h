...
public:
	...
	void setParameters(int min_line_length, double lineFitErrThreshold);

	Simplified_EDLines(int drows, int dcols);
	int runSimplified_EDLines(unsigned char* _imgG, int irows, int icols, int needGaussBlur)
	{
		cv::Mat imgG(irows, icols, CV_8UC1, _imgG);
		runSimplified_EDLines(imgG, needGaussBlur? true: false);

		return LineSegments.size();
	}
	void runSimplified_EDLines(cv::Mat &ImgG, bool need_gaussblur = false);
	//void runSimplified_EDLines_tbb(cv::Mat &ImgG);
	std::vector< cv::Vec4i > LineSegments;

	void UpdateLineSegments(signed int *_linesegs);
	...
	void drawLineSegments4i(unsigned char *_imgC, int irows, int icols);
