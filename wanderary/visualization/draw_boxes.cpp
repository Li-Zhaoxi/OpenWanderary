...
void DrawBoxes2D(const std::vector<wdr::Box2D> &boxes, cv::Mat *img) {
  for (const auto &box : boxes) {
    ...
    cv::rectangle(*img, cv::Point(box.x_min, box.y_min - 20),
                  ...
                  cv::Scalar(0, 0, 255), -1);
    cv::putText(*img, std::to_string(box.label.id),
...
void Box2DDrawer::draw(const std::vector<wdr::Box2D> &boxes, cv::Mat *img) {
  for (const auto &box2d : boxes) {
    CHECK_LT(box2d.label.id, class_num_);
    CHECK_GE(box2d.label.id, 0);
    const auto &color = colors_[box2d.label.id];
    ...
    if (class_names_.size() > 0) {
      oss << class_names_[box2d.label.id] << ": " << std::fixed
          << std::setprecision(2) << box2d.score;
    } else {
      oss << box2d.label.id << ": " << std::fixed << std::setprecision(2)
          << box2d.score;
    }
